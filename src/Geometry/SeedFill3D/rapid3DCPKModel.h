 ...
 * @param z			image slice
 */
static inline void clearRange(voxelGrid & cpkModel, int xl, int xr, int y, int z) {
	for (int i = xl; i <= xr; i++)
		cpkModel.clearVoxel(i, y, z);
};
/**
...
static inline void obtainUnfilledValidRange(voxelGrid const & imageData, voxelGrid const & cpkModel, voxel const & seed,
		int & xl, int & xr) {
	xr = seed.ix;
	while ((xr < imageData.length) && !imageData.getVoxel(xr, seed.iy, seed.iz) && cpkModel.getVoxel(xr, seed.iy, seed.iz)) {
		++xr;
	}
	--xr;
	xl = seed.ix;
	while ((xl > -1) && !imageData.getVoxel(xl, seed.iy, seed.iz) && cpkModel.getVoxel(xl, seed.iy, seed.iz)) {
		--xl;
	}
	++xl;
};
 ...
 */
static inline bool checkAndFillRange(voxelGrid const & imageData, voxelGrid & cpkModel, int xpl, int xpr, int y, int z,
		IdList & dList, int & xl, int & xr) {
	for (int i = xpl; i <= xpr; ++i) {
		if (!imageData.getVoxel(i, y, z) && cpkModel.getVoxel(i, y, z)) { // if a valid seed is found
			...
			obtainUnfilledValidRange(imageData, cpkModel, voxel(i, y, z), xl, xr);
			//extract and fill the range [xl,xr], mark the seeds and write the ID of them into dList
			clearRange(cpkModel, xl, xr, y, z);
...
static inline void seedFill2D(voxelGrid const & imageData, voxelGrid & cpkModel, voxel const & seed, IdList & seedList) {
	...
	obtainUnfilledValidRange(imageData, cpkModel, seed, xl, xr);
	// extract and fill the range [xl,xr],
	clearRange(cpkModel, xl, xr, seed.iy, seed.iz);
	...
	while (!stack2D.empty()) { // if the 2D stack is not empty
		...
		while (checkAndFillRange(imageData, cpkModel, xpl, xpr, y, seed.iz, seedList, xl, xr)) { //if the range [xpl, xpr] is valid
			if (xr < xpr - 1) { //it maybe exist other valid and unfilled ranges on the same scan line
				for (int i = xr + 2; i <= xpr; ++i) { // search the leftmost seed of the next valid range, which locates at the same scan line
					if (!imageData.getVoxel(i, y, seed.iz) && cpkModel.getVoxel(i, y, seed.iz)) { /* a valid seed exists */
			...
			// rollback to the previous scan line
				...
				for (int i = xl; i <= xpl - 2; ++i) {
					if (!imageData.getVoxel(i, old_y, seed.iz) && cpkModel.getVoxel(i, old_y, seed.iz)) { /* a valid seed exists */
...
static inline bool searchNeighborSlice(voxelGrid const & imageData, voxelGrid const & cpkModel, IdList & seed_list,
	...
	for (id = begin; id != end; ++id) {
		for (int ix = id->xl; ix <= id->xr; ix += leap_var) {
			if (!imageData.getVoxel(ix, id->y, z) && cpkModel.getVoxel(ix, id->y, z)) {
				sx = ix;
				sy = id->y;
				/*
...
static inline void cpkModelFill(voxelGrid const & imageData, voxelGrid & cpkModel, voxel const & seed, int leap_var = 1) { // the pseudocode of proposed algorithm — 2009
	...
	while (!stack3D.empty()) {
		...
		while(searchNeighborSlice(imageData, cpkModel, currentSeeds, new_z, leap_var, sx, sy)){ //if the region is valid
			IdList newSeeds;
			...
			seedFill2D(imageData, cpkModel, voxel(sx, sy, new_z), newSeeds);
			//continue to search other new unfilled regions in the region (Plist_mapped)
			while (searchNeighborSlice(imageData, cpkModel, currentSeeds, new_z, leap_var, sx, sy)){
				//extract and fill these new unfilled regions
				IdList otherSeeds;
				seedFill2D(imageData, cpkModel, voxel(sx, sy, new_z), otherSeeds);
			...
			while (searchNeighborSlice(imageData, cpkModel, newSeeds,	new_z - n.direction_z, leap_var, sx, sy)) {
				IdList otherSeeds;
				//extract and fill these new unfilled regions
				seedFill2D(imageData, cpkModel, voxel(sx, sy, new_z - n.direction_z), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
