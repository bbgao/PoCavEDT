 ...
 * @param z			image slice
 */
static inline void clearRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & surface, int xl, int xr, int y, int z) {
	for (int i = xl; i <= xr; i++) {
		voxel nb, v(i, y, z);
		for (int ii = 0; ii < 4; ++ii) {
			nb = v + voxel_offset::neighbours[ii];
			if (nb.iy >= 0 && nb.iy < cpkModel.width
					&& nb.iz >= 0 && nb.iz < cpkModel.height
					&& cpkModel.getVoxel(nb))
				surface.setVoxel(nb);
		}
		temp.clearVoxel(v);
	}
	if (xl > 0)
		surface.setVoxel(xl - 1, y, z);
	if (xr < surface.length - 1)
		surface.setVoxel(xr + 1, y, z);
};
...
static inline void obtainUnfilledValidRange(voxelGrid const & cpkModel, voxelGrid const & temp, voxel const & seed,
		int & xl, int & xr) {
	xr = seed.ix;
	while ((xr < cpkModel.length) && !cpkModel.getVoxel(xr, seed.iy, seed.iz) && temp.getVoxel(xr, seed.iy, seed.iz)) {
		++xr;
	}
	--xr;
	xl = seed.ix;
	while ((xl > -1) && !cpkModel.getVoxel(xl, seed.iy, seed.iz) && temp.getVoxel(xl, seed.iy, seed.iz)) {
		--xl;
	}
	++xl;
};
 ...
 */
static inline bool checkAndFillRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & surface, int xpl, int xpr, int y, int z,
		IdList & dList, int & xl, int & xr) {
	for (int i = xpl; i <= xpr; ++i) {
		if (!cpkModel.getVoxel(i, y, z) && temp.getVoxel(i, y, z)) { // if a valid seed is found
			...
			obtainUnfilledValidRange(cpkModel, temp, voxel(i, y, z), xl, xr);
			//extract and fill the range [xl,xr], mark the seeds and write the ID of them into dList
			clearRange(cpkModel, temp, surface, xl, xr, y, z);
...
static inline void seedFill2D(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & surface, voxel const & seed, IdList & seedList) {
	...
	obtainUnfilledValidRange(cpkModel, temp, seed, xl, xr);
	// extract and fill the range [xl,xr],
	clearRange(cpkModel, temp, surface, xl, xr, seed.iy, seed.iz);
	...
	while (!stack2D.empty()) { // if the 2D stack is not empty
		...
		while (checkAndFillRange(cpkModel, temp, surface, xpl, xpr, y, seed.iz, seedList, xl, xr)) { //if the range [xpl, xpr] is valid
			if (xr < xpr - 1) { //it maybe exist other valid and unfilled ranges on the same scan line
				for (int i = xr + 2; i <= xpr; ++i) { // search the leftmost seed of the next valid range, which locates at the same scan line
					if (!cpkModel.getVoxel(i, y, seed.iz) && temp.getVoxel(i, y, seed.iz)) { /* a valid seed exists */
			...
			// rollback to the previous scan line
				...
				for (int i = xl; i <= xpl - 2; ++i) {
					if (!cpkModel.getVoxel(i, old_y, seed.iz) && temp.getVoxel(i, old_y, seed.iz)) { /* a valid seed exists */
...
static inline bool searchNeighborSlice(voxelGrid const & cpkModel, voxelGrid const & temp, IdList & seed_list,
	...
	for (id = begin; id != end; ++id) {
		for (int ix = id->xl; ix <= id->xr; ix += leap_var) {
			if (!cpkModel.getVoxel(ix, id->y, z) && temp.getVoxel(ix, id->y, z)) {
				sx = ix;
				sy = id->y;
				/*
 ...
 * 					valid seed
 */
static inline void outerSurfaceExtract(voxelGrid const & cpkModel, voxelGrid & surface, voxel const & seed, int leap_var = 1) { // the pseudocode of proposed algorithm — 2009
	...
	while (!stack3D.empty()) {
		...
		while(searchNeighborSlice(cpkModel, temp, currentSeeds, new_z, leap_var, sx, sy)){ //if the region is valid
			IdList newSeeds;
			...
			seedFill2D(cpkModel, temp, surface, voxel(sx, sy, new_z), newSeeds);
			//continue to search other new unfilled regions in the region (Plist_mapped)
			while (searchNeighborSlice(cpkModel, temp, currentSeeds, new_z, leap_var, sx, sy)){
				//extract and fill these new unfilled regions
				IdList otherSeeds;
				seedFill2D(cpkModel, temp, surface, voxel(sx, sy, new_z), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
				if (new_z < cpkModel.height - 1)
			...
			while (searchNeighborSlice(cpkModel, temp, newSeeds, new_z - n.direction_z, leap_var, sx, sy)) {
				IdList otherSeeds;
				//extract and fill these new unfilled regions
				seedFill2D(cpkModel, temp, surface, voxel(sx, sy, new_z - n.direction_z), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
				if (new_z - n.direction_z < cpkModel.height - 1)
			...
			new_z += n.direction_z;
			if (new_z < 0 || new_z == cpkModel.height)
				break;
		}
	}
};
