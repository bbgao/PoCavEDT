 ...
 * @param z			image slice
 */
static inline void fillRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output,
...
//				output.setVoxel(nb);
//		}
		output.setVoxel(i, y, z);
		temp.clearVoxel(i, y, z);
	}
//	if (xl > 0)
//		output.setVoxel(xl - 1, y, z);
//	if (xr < output.length - 1)
//		output.setVoxel(xr + 1, y, z);
};
/**
 ...
 */
static inline void obtainUnfilledValidRange(voxelGrid const & temp, voxel const & seed, int & xl, int & xr) {
	xr = seed.ix;
 ...
 */
static inline bool checkAndFillRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output, int xpl, int xpr, int y, int z,
	...
	for (int i = xpl; i <= xpr; ++i) {
		if (temp.getVoxel(i, y, z)) { // if a valid seed is found
			...
			obtainUnfilledValidRange(temp, voxel(i, y, z), xl, xr);
			//extract and fill the range [xl,xr], mark the seeds and write the ID of them into dList
			fillRange(cpkModel, temp, output, xl, xr, y, z);
...
static inline void pocketExtract2D(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output,
	...
	obtainUnfilledValidRange(temp, seed, xl, xr);
	// extract and fill the range [xl,xr],
	fillRange(cpkModel, temp, output, xl, xr, seed.iy, seed.iz);
	...
	while (!stack2D.empty()) { // if the 2D stack is not empty
		...
		while (checkAndFillRange(cpkModel, temp, output, xpl, xpr, y, seed.iz, seedList, xl, xr)) { //if the range [xpl, xpr] is valid
 ...
 * 					valid seed
 */
static inline void poCavExtract3D(voxelGrid const & imageData, voxelGrid & temp, voxelGrid & output,
	...
	while (!stack3D.empty()) {
		...
		while(searchNeighborSlice(temp, currentSeeds, new_z, leap_var, sx, sy)){ //if the region is valid
			...
			pocketExtract2D(imageData, temp, output, voxel(sx, sy, new_z), newSeeds);
			...
			while (searchNeighborSlice(temp, currentSeeds, new_z, leap_var, sx, sy)){
				...
				IdList otherSeeds;
				pocketExtract2D(imageData, temp, output, voxel(sx, sy, new_z), otherSeeds);
			...
			while (searchNeighborSlice(temp, newSeeds, new_z - n.direction_z, leap_var, sx, sy)) {
				IdList otherSeeds;
				//extract and fill these new unfilled regions
				pocketExtract2D(imageData, temp, output, voxel(sx, sy, new_z - n.direction_z), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
